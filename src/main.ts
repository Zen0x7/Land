...
const DISCOVERY_EVENT = 'Discovery';
...
interface KnownNodeStoreState {
  nodesByIdentifier: Record<string, KnownNode>;
}

const useKnownNodeStore = defineStore('knownNodeStore', {
  state: (): KnownNodeStoreState => {
    return {
      nodesByIdentifier: {},
    };
  },
  actions: {
    upsertNode(node: KnownNode): void {
      this.nodesByIdentifier[node.id] = node;
    },
    getNodes(): KnownNode[] {
      return Object.values(this.nodesByIdentifier);
    },
  },
});
...
const isValidUuidV4 = (value: string): boolean => {
  const uuidV4Pattern =
    /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu;
  return uuidV4Pattern.test(value);
};

const createSocketAddress = (host: string, port: number): string => {
...
class LandApp implements App {
  ...
  private connectToKnownNode(node: KnownNode): void {
    ...
    for (let index = 0; index < missingConnectionCount; index += 1) {
