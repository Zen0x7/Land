...
import { formatKilobytesPerSecond, formatMegabytes, formatMilliseconds, formatTimestamp, } from '../../utilities/formatters.js';
    ...
    computed: {
        selectedNodeMetrics() {
            const selectedNodeConnections = this
                .selectedNodeConnections;
            const readKilobytesPerSecond = selectedNodeConnections.reduce((sum, connection) => sum + connection.metrics.readKilobytesPerSecond, 0);
            const writeKilobytesPerSecond = selectedNodeConnections.reduce((sum, connection) => sum + connection.metrics.writeKilobytesPerSecond, 0);
            return {
                readKilobytesPerSecond,
                writeKilobytesPerSecond,
                totalKilobytesPerSecond: readKilobytesPerSecond + writeKilobytesPerSecond,
            };
        },
    },
    methods: {
