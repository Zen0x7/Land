    ...
    computed: {
        chartRows() {
            ...
            const maximumValue = connections.reduce((maximum, connection) => {
                const value = connection.metrics.readKilobytesPerSecond +
                    connection.metrics.writeKilobytesPerSecond;
                return Math.max(maximum, value);
            }, 0);
            return connections.map((connection) => {
                const combinedBandwidth = connection.metrics.readKilobytesPerSecond +
                    connection.metrics.writeKilobytesPerSecond;
                return {
                    ...connection,
                    combinedBandwidth,
                    percentage: maximumValue > 0 ? (combinedBandwidth / maximumValue) * 100 : 0,
                };
            });
        },
    },
